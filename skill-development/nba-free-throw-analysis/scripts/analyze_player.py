...
def classify_game(game_id, segment, payload, player_id, game_meta=None):
    ...
    for i, play in enumerate(plays):
        ...
        if "free throw" not in (type_text + " " + text).lower() or str(player_id) not in _participants(play):
            continue
...
def write_outputs(rows, output, player_name, season_label, validations=None):
    output.mkdir(parents=True, exist_ok=True)
    fields = list(rows[0]) if rows else []
    with (output / "free_throw_attempts.csv").open("w", newline="", encoding="utf-8-sig") as handle:
    ...
    lines = [f"# {player_name} Advanced Free Throws — {season_label}", "", "Regular season, Play-In, and playoffs", "", "| Split | Makes | Attempts | Misses | FT% |", "|---|---:|---:|---:|---:|"]
...
def run(base: Path, player_id: str, player_name: str, season_end_year: int, output: Path):
    season_label = f"{season_end_year - 1}-{str(season_end_year)[-2:]}"
    cache = base / "data" / "raw" / "espn" / str(player_id) / str(season_end_year)
    log = fetch_json(f"https://site.web.api.espn.com/apis/common/v3/sports/basketball/nba/athletes/{player_id}/gamelog?season={season_end_year}", cache / "gamelog.json")
    ...
    for game_id in sorted(eligible, key=lambda x: log.get("events", {}).get(x, {}).get("gameDate", "")):
        ...
        try:
            ...
            game_rows = classify_game(game_id, segment, summary, player_id, meta)
    ...
    write_outputs(rows, output, player_name, season_label, validations)
    mismatches = [v for v in validations if not v["passed"]]
    manifest = {"player_name": player_name, "espn_player_id": str(player_id), "season": season_label, "games_discovered": len(eligible), "games_failed": failures, "games_reconciled": len(validations)-len(mismatches), "reconciliation_mismatches": mismatches, "attempts": len(rows), "complete": not failures and not mismatches, "source": "ESPN public play-by-play and box-score game log"}
    (output / "manifest.json").write_text(json.dumps(manifest, indent=2), encoding="utf-8")
    return manifest


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Classify every free throw for one NBA player-season.")
    parser.add_argument("--player-id", required=True, help="ESPN athlete ID")
    parser.add_argument("--player-name", required=True)
    parser.add_argument("--season-end-year", required=True, type=int, help="2026 means the 2025-26 season")
    parser.add_argument("--work-dir", type=Path, default=Path.cwd())
    parser.add_argument("--output", type=Path, required=True)
    args = parser.parse_args()
    print(json.dumps(run(args.work_dir, args.player_id, args.player_name, args.season_end_year, args.output), indent=2))
